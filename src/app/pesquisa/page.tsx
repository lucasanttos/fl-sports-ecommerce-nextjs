  ...
  useEffect(() => {
    const searchAll = async () => {
      try {
        ...
        // Roda a sua filtragem original em cima do array unificado
        const filtered = baseProducts.filter(product => 
          product.name.toLowerCase().includes(query) || 
          (product.category || product.category_slug || '').toLowerCase().includes(query)
        );
      ...
      {filteredProducts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {filteredProducts.map((product, index) => (
            <ScrollReveal key={`search-prod-${product.slug || product.id}-${index}`} delay={(index % 4) * 0.1} direction="up">
